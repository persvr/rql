...
return {
	isValid: function(query, template){
		return isValid(parseQuery(query), parseQuery(template));
	},
...
function isValid(query, template){
	console.log("comparing ", query, template);
	if(query && query.name){
		console.log("operators[query.name] && operators[query.name].commutativ", query.name, operators[query.name] && operators[query.name].commutative);
		return query.name == template.name &&
			operators[query.name] && operators[query.name].commutative ? 
	...
	if(template && template.variable){
		template.name = "and";
		console.log("execute ", template.toString());
		return executeQuery(template, {}, [query]).length;
	}
}
