...
function parse(/*String|Object*/query, parameters){
	...
	var leftoverCharacters = query.replace(/([&\|,]?)(\}[\?\*\+]?|\{[\-\w%\._]+\:?|[\+\*\$\-:\w%\._]*)(\()?(\)[\?\*\+]?)?/g,
		...
		function(t, delim, propertyOrValue, openParan, closeParan){
			//console.log("parse",t, delim, propertyOrValue, openParan, closeParan);
			if(delim){
				if(delim === "&"){
					setConjunction("and");
				}
			...
			if(propertyOrValue === "}"){
				if(!term.parent || !term.variable){
					throw new URIError("Closing curly without an opening curly" + term.parent + term.name);
				}
				propertyOrValue = "value" in term ? term.value : term;
				term = term.parent;
			}
			if(propertyOrValue.charAt && propertyOrValue.charAt(0) == '{'){
				// a variable that can be substituted
				var colon = propertyOrValue.indexOf(":");
				propertyOrValue = propertyOrValue.substring(1, colon > -1 ? colon : propertyOrValue.length);
				var newTerm = new exports.Variable();
				newTerm.variable = propertyOrValue;
				newTerm.parent = term;
				if(parameters && parameters.hasOwnProperty(propertyOrValue)){
					// parameter found, substitute
					newTerm.value = parameters[propertyOrValue];
				}
				term = newTerm;
			}
			else if(openParan){
			...
			if(closeParan){
				...
				if(closeParan.charAt(1) == "?"){
					term.optional = true;
				}
				term.parent.args.push(isArray ? term.args : term);
				term = term.parent;
			}			
			return "";
		});
