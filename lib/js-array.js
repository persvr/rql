...
require.def||(require.def=function(deps, factory){module.exports = factory(exports, require("./parser"));});
require.def(["exports", "./parser"], function(exports, parser){
...
function query(query, options, target){
	...
	function queryToJS(value){
		if(value && typeof value === "object" && !(value instanceof Array)){
			...
			if(jsOperator){
				if (jsOperator === '===' && value.args[1].indexOf('re:') === 0){
					jsOperator = '.match(/'+value.args[1].substring(3)+'/i)&&""===';
					value.args[1] = '';
					//console.log('OP', jsOperator, queryToJS(value.args[1]), value.args[1]);
				}
				...
				var s = "(function(){var filtered = []; for(var i = 0, length = this.length; i < length; i++){var item = this[i];if(item." + value.args[0] + jsOperator + queryToJS(value.args[1]) + "){filtered.push(item);}} return filtered;})";
				//console.log('FUNC', s);
				return s;
			}else{
